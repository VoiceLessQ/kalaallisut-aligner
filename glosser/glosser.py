    ...
    def output_html(self, glossed_items):
        ...
        for item in glossed_items:
            if item['type'] == 'punctuation':
                ...
                html.append(f'<div class="word unknown"><div class="surface">{item["surface"]}</div><div>❌ Unknown</div></div>')
            else:
                ...
                if item['all_analyses'] > 1:
                    html.append(f'<div style="font-size:12px;color:#999;">({item["all_analyses"]} analyses)</div>')
                html.append('</div>')
